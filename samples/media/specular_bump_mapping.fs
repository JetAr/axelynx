...
float specular = pow(clamp(dot(reflect(-lVec, bump), vVec), 0.0, 1.0), texture2D(texture1, fragmentuv).a );
color = texture2D(texture1, fragmentuv);//((diffuse + specular)*base) * att;
}
