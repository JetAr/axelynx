...
function AddConfig(proj, strProjectName)
{
	try {
		...
		CLTool.AdditionalIncludeDirectories = '$(AXELYNX_SDK)';
		CLTool.UsePrecompiledHeader = pchOption.pchGenerateAuto;
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMD")
		...
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() != "DLL")
		    CLTool.PreprocessorDefinitions = "AXELYNX_STATIC;"
		var LinkTool = config.Tools('VCLinkerTool');
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "DLL")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\Debug\\axelynx$(PlatformArchitecture)_d.lib';
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMT")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\DebugMT\\axelynx_d.lib opengl32.lib winmm.lib';
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMD")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\DebugMD\\axelynx_d.lib opengl32.lib winmm.lib';
		LinkTool.GenerateDebugInformation = true;

		if (wizard.FindSymbol('SAMPLE_RADIO_OPTION_CONSOLE').toString() == "true")
		...
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "DLL") {
		    ...
		    PostBuildEventTool.CommandLine = 'copy "$(AXELYNX_SDK)bin\\cl\\$(PlatformName)\\Debug\\axelynx$(PlatformArchitecture)_d.dll" "$(OutDir)"';
		}
		...
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() != "DLL")
		    CLTool.PreprocessorDefinitions = "AXELYNX_STATIC;"
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMD")
		    CLTool.RuntimeLibrary = runtimeLibraryOption.rtMultiThreadedReleaseDLL;
		else
		    CLTool.RuntimeLibrary = runtimeLibraryOption.rtMultiThreadedRelease;
		// TODO: Add compiler settings
		CLTool.AdditionalIncludeDirectories = '$(AXELYNX_SDK)';
		CLTool.UsePrecompiledHeader = pchOption.pchGenerateAuto;
		var LinkTool = config.Tools('VCLinkerTool');
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "DLL")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\Release\\axelynx$(PlatformArchitecture).lib';
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMT")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\ReleaseMT\\axelynx.lib opengl32.lib winmm.lib';
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "StaticMD")
		    LinkTool.AdditionalDependencies = '$(AXELYNX_SDK)lib\\cl\\$(PlatformName)\\ReleaseMD\\axelynx.lib opengl32.lib winmm.lib';
		...
		if (wizard.FindSymbol('ENGINE_LINKIN_TYPE').toString() == "DLL") {
		    var PostBuildEventTool = config.Tools('VCPostBuildEventTool');
		    PostBuildEventTool.CommandLine = 'copy "$(AXELYNX_SDK)bin\\cl\\$(PlatformName)\\Release\\axelynx$(PlatformArchitecture).dll" "$(OutDir)"';
		}
	}
	catch(e)
	{
...
if (wizard.FindSymbol('SAMPLE_CHECKBOX_PCH').toString() == "true") {
    if (strName == 'stdafx.h')
        strTarget = 'stdafx.h';
